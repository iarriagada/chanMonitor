...
if __name__ == '__main__':
    ...
    mcs_track_err = ped.tracking_filter(recData['mc:azPosError'],
                                        recData['mc:inPositionAz'])

    mc_terror_fft = ped.fft_generator(mcs_track_err)
    ...
    mc_trk_err = DataAx(mcs_track_err,
                      'xkcd:aquamarine',
                      label='mc:azPosError',
                      ylabel='Position [deg]',
                      linewidth=1.25)

    mc_trk_err_fft = DataAx(mc_terror_fft,
                      'xkcd:apple green',
    ...
    mcs_track_err_hist = DataAx(mcs_track_err[1],
                                     'r',
                                     label='mcs track err hist',
                                     ylabel="No Samples\n[un]",
                                     xlabel="Error [deg]",
                                     histbins=30,
                                limsbins=lim_bin_trk,
                                     linewidth=2.50)
    ...
    plts.Axe['c1']['mc_azPos'] = DataAx.update_axe(mc_azPos,
                                                   shaxname='mc_azDmd')
    plts.Axe['c1']['mc_azTrkErr'] = mc_trk_err
    plts.Axe['c2']['mc_trk_err_hist'] = mcs_track_err_hist

    plts.Axe['c3']['mc_trk_err_fft'] = mc_trk_err_fft
    # plts.Axe['c1']['mc_azErr'] = mc_azErr
    # plts.Axe['c1']['mc_azTrkErr'] = DataAx.update_axe(mc_trk_err,
                                                      # shaxname='mc_azErr')
